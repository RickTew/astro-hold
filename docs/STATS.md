...
**S23 changes since S22d (NOT browser-validated; tsc + vite build clean):**
- **Human faction (third faction).** `Faction = 'robot' | 'cyborg' | 'human'`, decoupled from role (the picker's "Change factions" pill cycles all 6 distinct matchups; AI plays the faction on the card you did not pick). Faction-aware sprite art via `FACTION_ART` in `SpriteUnit.ts` (art key resolves per faction; gameplay TYPE unchanged so stats are identical = NO balance change). Human roster reuses existing attacker stat blocks: **WARRIOR** = `cannon` (human_warrior art), **MARINE** = `doublegun` (human_marine art), **MEDIC** = `medic` (human_medic art). Music: human reuses the cyborgs track (placeholder). Voice still side-based (placeholder). No human defensive structures yet (human defender uses shared robot structures).
- **Cyborg Medic re-arted** with Cyborg_Healer.zip; the prior medic art moved to `/sprites/human_medic` as the Human medic.
- **Human gunners' aim-fire.** Warrior + Marine ship no `shoot` clip but an `aim` clip; `playAttackAnim` now falls back to `aim` as a quick pre-fire reaction (returns to idle once done, unlike the Sniper which HOLDS its crouched aim and fires next turn).
- **Cyborg Nerd / HACKER unit + hack mechanic** (see roster table + `project_hacker_unit`). New combat mechanic: turns robots into 3-turn traitors. Stats untuned.
...
| **Hacker** (Cyborg Nerd) | 80 | 60 | 60 | 0 | 200 (hack) | 320 | — | 3 | **2 (= hacks)** | Saboteur, NO gun. Within range, plays the ipad cast and HACKS an enemy robot piece (direct-fire tower `turret`/`laser`/`sentry`, or mobile defender `dog`/`repair`). For **3 reveals** that piece turns traitor: it attacks other robots, cyborgs ignore it. **One-way** (loyal robots don't retaliate). Core is never a hack/target. Excluded targets: Phaser/cannon (beam scans cyborgs), bomber/signal (special actions), walls/mines/shields (no weapon). Out of hacks → retreats east. Stats untuned. See `project_hacker_unit`. |
